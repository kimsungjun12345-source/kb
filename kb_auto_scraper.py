...
class KBAutoScraper:
    ...
    async def collect_essential_data(self, page, age, gender, birthdate):
        try:
            ...
            # 체크박스 정보 (특약) - 개별 활성화 테스트
            checkboxes = await page.query_selector_all("input[type='checkbox']")
            base_premium = None

            # 기본 보험료 먼저 수집
            base_premium_info = await self.get_current_premium_amount(page)
            if base_premium_info:
                base_premium = base_premium_info

            for i, checkbox in enumerate(checkboxes[:15]):
                try:
                    ...
                    if parent_text:
                        ...
                        if not is_checked:
                            try:
                                ...
                                clause_info["premium_with_clause"] = new_premium
                                ...
                                logger.warning(f"특약 {i} 테스트 중 오류: {str(e)}")

                        result_data["special_clauses"].append(clause_info)

                except Exception as e:
                    logger.warning(f"체크박스 {i} 처리 중 오류: {str(e)}")
                    continue
    ...
    async def scrape_test_range(self, start_age=20, end_age=25):
        """테스트 범위 데이터 수집 (20-25세)"""
        genders = ["남성", "여성"]
        ages = list(range(start_age, end_age + 1))
        total_cases = len(ages) * len(genders)

        logger.info(f"테스트 데이터 수집 시작: {start_age}-{end_age}세, {len(genders)}개 성별")
        ...
        logger.info(f"테스트 수집 완료: 성공 {successful_count}, 실패 {failed_count}")
        return self.data
    ...
    async def save_final_data(self):
        try:
            ...
            # 전체 데이터
            main_filename = f"kb_insurance_test_{timestamp}.json"
            ...
            # CSV 형태로도 저장
            csv_filename = f"kb_insurance_test_{timestamp}.csv"
            ...
            if self.data:
                ...
                for i, record in enumerate(self.data[:3]):
                    print(f"  {i+1}. {record['age']}세 {record['gender']} - 보험료 {len(record.get('premium_info', []))}개, 특약 {len(record.get('special_clauses', []))}개")
    ...
    async def run_auto_collection(self):
        """자동 수집 실행 (테스트 모드)"""
        logger.info("KB생명 보험 자동 데이터 수집 시작 (테스트 모드: 20-25세)")
        await self.scrape_test_range(20, 25)

async def main():
    scraper = KBAutoScraper()
    await scraper.run_auto_collection()
