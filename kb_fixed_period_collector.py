"""
KB생명 보험기간/납입기간별 가격 변화 데이터 수집기 - 수정됨
...
class KBFixedPeriodCollector:
    """KB생명 보험기간/납입기간별 가격 수집기 - 수정됨"""
    ...
    async def collect_period_data(self):
        """보험기간/납입기간별 데이터 수집"""
        ...
        try:
            ...
            print(f"페이지 로딩 완료: {await self.page.title()}")
                    ...
                    for combination in self.period_combinations:
                        ...
                        try:
                            ...
                            if "birthdate" in fields:
                                ...
                                await fields["birthdate"].fill(birthdate)
                                    ...
                                    for wait_attempt in range(5):
                                        try:
                                            ...
                                            if "원/월" in body_text or "월보험료" in body_text or "보험료" in body_text:
                                                print(f"  계산 완료 확인됨 (시도 {wait_attempt + 1})")
                                                break
                                            else:
                                                print(f"  결과 대기 중... (시도 {wait_attempt + 1}/5)")
                                                await self.page.wait_for_timeout(3000)
                                        except Exception:
                                            await self.page.wait_for_timeout(3000)
    ...
    async def run(self):
        ...
        try:
            ...
            # 데이터 수집
            success = await self.collect_period_data()
...
async def main():
    """비동기 메인 함수"""
    collector = KBFixedPeriodCollector()
    await collector.run()
